...
/*****************************************************************
 * HQC‑256 Vector Sampling Benchmark
 ...
 *****************************************************************/
 ...
 #include "shake_prng.h"    // PQCLEAN_HQC256_CLEAN_seedexpander_*
 #include "vector.h"        // single_bit_mask(), compare_u32(), m_val[], PQCLEAN_HQC256_CLEAN_vect_set_random_fixed_weight()
 #include "parameters.h"    // PARAM_N, PARAM_OMEGA_R, VEC_N_SIZE_64
 #include "hqc_reducer.h"   // hqc256_use_barrett(), hqc256_use_mulshift()
                                 ...
                                 uint64_t       out[VEC_N_SIZE_64])
 {
     ...
     PQCLEAN_HQC256_CLEAN_seedexpander_init(&ctx, (uint8_t*)&seed, 4);
     PQCLEAN_HQC256_CLEAN_vect_set_random_fixed_weight(&ctx, out, VEC_W);
     PQCLEAN_HQC256_CLEAN_seedexpander_release(&ctx);
 }
                                       ...
                                       uint64_t out[VEC_N_SIZE_64])
 {
     ...
     for(int i=0;i<VEC_W;i++){
         uint8_t buf[4];
         PQCLEAN_HQC256_CLEAN_seedexpander(&ctx,buf,4);
...
// Given ⟨a⟩_bool where a = a0⊕a1, produce ⟨a⟩_arith with sum = a
 ...
 int main(void){
     puts("\nHQC‑256 Vector Sampling (ω=114) Benchmark\n");
