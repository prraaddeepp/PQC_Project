...
/*****************************************************************
 * HQC‑128 Vector Sampling Benchmark
 ...
 *****************************************************************/
 ...
 #include "vector.h"        // single_bit_mask(), compare_u32(), m_val[], PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight()
 #include "parameters.h"    // PARAM_N, PARAM_OMEGA_R, VEC_N_SIZE_64
 #include "hqc_reducer.h"   // hqc128_use_barrett(), hqc128_use_mulshift()
 
 // bench parameters
 #define VEC_W    PARAM_OMEGA_R   // 75
                                 ...
                                 uint64_t       out[VEC_N_SIZE_64])
 {
     ...
     PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight(&ctx, out, VEC_W);
...
// Given ⟨a⟩_bool where a = a0⊕a1, produce ⟨a⟩_arith with sum = a
 ...
 int main(void){
     puts("\nHQC‑128 Vector Sampling (ω=75) Benchmark\n");
